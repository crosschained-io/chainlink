...
func (eb *EthBroadcaster) handleInProgressEthTx(etx EthTx, attempt EthTxAttempt, initialBroadcastAt time.Time) error {
	...
	if etx.Simulate {
		...
		defer cancel()
		if err := simulateTransaction(simulationCtx, eb.ethClient, attempt, etx); err != nil {
			if jErr := eth.ExtractRPCError(err); jErr != nil {
				eb.logger.Errorw("BulletproofTxManager: Transaction reverted during simulation", "ethTxAttemptID", attempt.ID, "txHash", attempt.Hash, "err", err, "rpcErr", jErr.String())
			...
			logger.Warnw("BulletproofTxManager: Transaction simulation failed, will attempt to send anyway", "ethTxAttemptID", attempt.ID, "txHash", attempt.Hash, "err", err)
		}
		logger.Debugw("BulletproofTxManager: Transaction simulation succeeded", "ethTxAttemptID", attempt.ID, "txHash", attempt.Hash)
	}
